...
- Dummy `NotificationServiceClient` called when a TODO is created
- JaCoCo coverage verification with an 85% minimum line coverage rule
...
The project uses JaCoCo and fails the build if line coverage is below 85%. The HTML coverage report is generated at:
...
- `TodoController`: logs incoming API requests and successful API actions.
- `TodoService`: logs business operations such as create, fetch, update, delete, and invalid status transitions.

## Notification Service Simulation

`NotificationServiceClient` is a dummy Spring-managed service that simulates an external notification service. It is injected into `TodoService` using constructor injection and called when a new TODO is created.

Example log message:

```text
Notification sent for new TODO with id: 1 and title: Complete assignment
```
...
- `NotificationServiceClient.java`: Dummy client that simulates sending a notification after TODO creation.
...
- `TodoControllerTest.java`: Unit tests for REST controller behavior and JSON error responses.
- `DtoAndEntityTest.java`: Unit tests for DTO and entity behavior that support coverage verification.
- `TodoMapperTest.java`: Unit tests for manual DTO/entity mapping.
