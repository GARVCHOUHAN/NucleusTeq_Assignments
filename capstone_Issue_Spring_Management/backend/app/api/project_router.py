...
def create_project(
    ...
    result = ProjectService.create_project(project, current_user)
    return {
        **result.model_dump(),
...
def get_all_projects(current_user: dict = Depends(require_member)) -> list:
...
def update_project(
    ...
    result = ProjectService.update_project(
        project_id,
        updated_project.model_dump(exclude_unset=True),
        current_user
    )
    return {
        **result.model_dump(),
...
def delete_project(
    ...
    result = ProjectService.delete_project(project_id, current_user)
    return {
        **result.model_dump(),
...
def add_member(
    ...
    result = ProjectService.add_member(
        project_id,
        member.email,
        current_user
    )
...
def remove_member(
    ...
    result = ProjectService.remove_member(project_id, email, current_user)
    return {
        **result.model_dump(),
...
def get_assigned_projects(current_user: dict = Depends(require_member)) -> list:
